...
export default function SuperAdminPage() {
  ...
  const toggleActive = async (id: string, currentlyActive: boolean) => {
    try {
      await api.patch(`/super-admin/restaurants/${id}`, { isActive: !currentlyActive });
      toast.success(currentlyActive ? t('superAdmin.suspend') + ' ✓' : t('superAdmin.activate') + ' ✓');
      fetchData();
    } catch {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {stats?.activeRestaurants || 0}/{stats?.totalRestaurants || 0} {t('superAdmin.activeRestaurants').toLowerCase()}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            <DialogTrigger asChild>
              <Button size="sm">
                <Plus className="mr-1 h-4 w-4" /> {t('superAdmin.createRestaurant')}
              </Button>
            </DialogTrigger>
            <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder="contact@chezmoussa.ci"
                    />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label>{t('superAdmin.adminFirstName')} *</Label>
                    <Input
                  ...
                  <div className="space-y-1">
                    <Label>{t('superAdmin.adminLastName')} *</Label>
                    <Input
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder="admin@chezmoussa.ci"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label>{t('superAdmin.adminPassword')} *</Label>
                    <Input
                      type="password"
                ...
                <div className="space-y-1">
                  <Label>{t('superAdmin.adminPhone')}</Label>
                  <Input
                ...
                <Button className="w-full" onClick={createRestaurant} disabled={creating}>
                  {creating ? t('common.loading') : t('superAdmin.createRestaurant')}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
      ...
      {loading ? (
        ...
        <>
              ...
              <Card key={stat.label}>
                <CardContent className="flex items-center gap-4 p-6">
                  ...
                  <div>
                    ...
                    {'sub' in stat && stat.sub && <p className="text-xs text-muted-foreground">{stat.sub}</p>}
                  </div>
                </CardContent>
              </Card>
            ))}
          ...
          <div className="space-y-3">
            <h2 className="text-lg font-semibold">{t('superAdmin.restaurants')} ({restaurants.length})</h2>
                    ...
                    <CardHeader className="pb-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className={planColors[r.plan] || ''}>
                            {r.plan}
                          </Badge>
                    ...
                    <CardContent className="space-y-3">
                      <div className="grid grid-cols-4 gap-2 text-center">
                        <div>
                          ...
                          <p className="text-[10px] text-muted-foreground">{t('superAdmin.users')}</p>
                        </div>
                        <div>
                          <p className="text-lg font-bold">{r.tables}</p>
                          <p className="text-[10px] text-muted-foreground">{t('superAdmin.tables')}</p>
                        </div>
                        <div>
                          <p className="text-lg font-bold">{r.menuItems}</p>
                          <p className="text-[10px] text-muted-foreground">{t('superAdmin.menuItems')}</p>
                        </div>
                        <div>
                          <p className="text-lg font-bold">{r.orders}</p>
                          <p className="text-[10px] text-muted-foreground">{t('superAdmin.orders')}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between border-t pt-2">
                        <div>
                          <p className="text-sm font-semibold">
                            ...
                            <span className="ml-1 text-xs font-normal text-muted-foreground">
                              ({r.todayOrders} {t('superAdmin.todayOrders').toLowerCase()})
                            </span>
                          </p>
                          <p className="text-[10px] text-muted-foreground">
                            {t('superAdmin.created')}: {new Date(r.createdAt).toLocaleDateString()}
                          </p>
                        </div>
                        <div className="flex gap-1">
                          ...
                          >
                            {r.isActive ? (
                              <><PowerOff className="mr-1 h-3.5 w-3.5" /> {t('superAdmin.suspend')}</>
                            ) : (
                              <><Power className="mr-1 h-3.5 w-3.5" /> {t('superAdmin.activate')}</>
                            )}
                          </Button>
                          <Button
                            size="sm"
