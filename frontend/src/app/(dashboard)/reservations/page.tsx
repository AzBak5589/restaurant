...
import { Plus, RefreshCw, CalendarClock, Clock, Users } from 'lucide-react';
...
interface Reservation {
  ...
  status: string;
  notes: string | null;
  table?: { number: string; zone: string };
}
...
export default function ReservationsPage() {
  ...
  const createReservation = async () => {
    ...
    try {
      ...
      setCreateOpen(false);
      setForm({ customerName: '', customerPhone: '', guestCount: '2', date: new Date().toISOString().split('T')[0], startTime: '19:00', notes: '' });
      fetchReservations();
    } catch {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">{reservations.length} reservations</p>
        </div>
          ...
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                ...
                <Button className="w-full" onClick={createReservation} disabled={creating}>
          ...
          {reservations.map((r) => (
            <Card key={r.id}>
              ...
              <CardContent className="space-y-3">
                ...
                {r.table && <Badge variant="secondary">Table {r.table.number}</Badge>}
                {r.notes && <p className="text-xs text-muted-foreground italic">{r.notes}</p>}
                <div className="flex gap-1.5">
                  {r.status === 'PENDING' && (
                    <>
                      <Button size="sm" variant="outline" className="flex-1" onClick={() => updateStatus(r.id, 'CONFIRMED')}>Confirm</Button>
                      <Button size="sm" variant="destructive" className="flex-1" onClick={() => updateStatus(r.id, 'CANCELLED')}>Cancel</Button>
                    </>
                  )}
                  {r.status === 'CONFIRMED' && (
                    <>
                      <Button size="sm" variant="outline" className="flex-1" onClick={() => updateStatus(r.id, 'SEATED')}>Seat</Button>
                      <Button size="sm" variant="secondary" className="flex-1" onClick={() => updateStatus(r.id, 'NO_SHOW')}>No Show</Button>
                    </>
                  )}
                  {r.status === 'SEATED' && (
                    <Button size="sm" variant="outline" className="flex-1" onClick={() => updateStatus(r.id, 'COMPLETED')}>Complete</Button>
                  )}
                </div>
              </CardContent>
            </Card>
